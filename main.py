...
# ✅ Ensure 'year' and 'branch' exist in all student records
import os, json
if os.path.exists(FILE_NAME):
    ...
    for student in students:
        student.setdefault("year", "Unknown")
        student.setdefault("branch", "Unknown")
...
async def register_student(
    ...
    student = {"name": name, "roll": roll, "year": year, "branch": branch}
...
async def delete_student(student_id: int = Form(...)):
    ...
    if 0 <= student_id < len(students):
        ...
        with open(FILE_NAME, "w") as f:
            json.dump(students, f, indent=4)
        return HTMLResponse(f"Deleted student: {deleted_student['name']}<br><a href='/dashboard'>Go Back</a>")
    else:
        return HTMLResponse("Student not found<br><a href='/dashboard'>Go Back</a>")
